...
def load_file(path:str) -> str:
    ...
    with open("/".join(re.split('/|\\\\',os.path.dirname(__file__))[:-1])+path,'r',encoding='utf-8') as content:
...
def load_bin_file(path:str) -> bytes:
    ...
    path = "/".join(re.split('/|\\\\',os.path.dirname(__file__))[:-1])+path
...
def render_page(path:str, args:dict) -> str:
    ...
    content = ''
    try:
        with open("/".join(re.split('/|\\\\',os.path.dirname(__file__))[:-1])+path,'r',encoding='utf-8') as page:
            content = '\n'.join(page.readlines())
